...
//
// CLAUDE.md anchor #3: no TR-181 / TR-098 default in core. The
...
// no throttle (matches the #28 Listener default).
//
...
// rawGroup is a single-instance object with no instance numbering —
// just a path prefix shared by N child leaves. Use this for spec
...
func mergeFiles(tree *Tree, files []*loadedFile) (mergedConfig, error) {
	...
	for _, lf := range files {
		...
		// When auth is enabled, every model-bearing field is REQUIRED —
		// no TR-181 fallbacks in core Go (CLAUDE.md anchor #3). Operators
	...
	// connectionRequest block above (CLAUDE.md anchor #3: no TR-181 /
	...
	for _, lf := range files {
		...
		periodicCfg = PeriodicInformPaths{Interval: raw.Interval, Enable: raw.Enable}
...
// Replaces the per-file applyProfile from the prior #16 implementation
