...
func TestLoadExampleTR181Gateway(t *testing.T) {
	t.Parallel()

	prof, err := paramtree.LoadProfile("../../profiles/example-tr181-gateway")
	if err != nil {
		t.Fatalf("LoadProfile: %v", err)
	}
	if v, err := prof.Tree.Get("Device.DeviceInfo.Manufacturer"); err != nil || v.Raw != "Generic Networks" {
		t.Errorf("Manufacturer = %+v err=%v", v, err)
	}
	// EasyMesh extender path materialized.
	if _, err := prof.Tree.Get("Device.WiFi.MultiAP.APDevice.1.Radio.1.AP.1.AssociatedDevice.1.MACAddress"); err != nil {
		t.Errorf("EasyMesh extender station path not materialized: %v", err)
	}
...
func TestLoadExampleTR098Gateway(t *testing.T) {
	t.Parallel()

	prof, err := paramtree.LoadProfile("../../profiles/example-tr098-gateway")
	if err != nil {
		t.Fatalf("LoadProfile: %v", err)
	}
	if v, err := prof.Tree.Get("InternetGatewayDevice.DeviceInfo.Manufacturer"); err != nil || v.Raw != "Generic Networks" {
		t.Errorf("Manufacturer = %+v err=%v", v, err)
	}
	// Vendor-extension extender path materialized.
	if _, err := prof.Tree.Get("InternetGatewayDevice.X_GENERIC_Extender.1.AssociatedDevice.1.AssociatedDeviceMACAddress"); err != nil {
		t.Errorf("X_GENERIC_Extender path not materialized: %v", err)
	}
