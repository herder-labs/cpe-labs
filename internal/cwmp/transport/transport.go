...
func (t *Transport) Send(ctx context.Context, body []byte) ([]byte, error) {
	...
	// 401 challenge: build auth, retry once.
	if resp.StatusCode == http.StatusUnauthorized && t.auth == nil {
		...
		auth, parseErr := parseChallenge(challenge, t.cfg.Username, t.cfg.Password, t.pool.cnonce)
		if parseErr != nil {
	...
	if resp.StatusCode == http.StatusUnauthorized {
		// Auth retry already attempted; surface the failure.
		return nil, t.httpError(resp, respBody, cpeerr.KindInvalidArgument)
	}
