...
func TestSPVReadOnly(t *testing.T) {
  ...
  <ParameterKey>k</ParameterKey>
	...
	var fe *cwmp.FaultError
	if !errors.As(err, &fe) || fe.Fault.FaultCode != 9008 {
		t.Errorf("expected fault 9008, got: %v", err)
	}
...
func TestSPVInvalidValue(t *testing.T) {
  ...
  <ParameterKey>k</ParameterKey>
	...
	var fe *cwmp.FaultError
	if !errors.As(err, &fe) || fe.Fault.FaultCode != 9007 {
		t.Errorf("expected fault 9007 (invalid value), got: %v", err)
	}
}
