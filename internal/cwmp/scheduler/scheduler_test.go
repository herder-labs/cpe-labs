...
func TestSchedulerSessionMuBlocksTick(t *testing.T) {
	t.Parallel()
	...
	// Hold SessionMu so the next tick is dropped.
	sessionMu.Lock()
	clock.Advance(30 * time.Second)
	select {
	case <-tickCh:
		t.Fatal("tick fired despite SessionMu held")
	case <-time.After(50 * time.Millisecond):
	}
	sessionMu.Unlock()
	...
	case <-tickCh:
	case <-time.After(time.Second):
		t.Fatal("tick did not fire after SessionMu released")
	}
}
